...
module.exports = {
		entry: ((filepathList) => {
				...
				filepathList.forEach((filepath) => {
						...
						entry[key] = isDev ? [filepath, "webpack-hot-middleware/client?reload=true"] :
								filepath;
				});
				return entry;
		...
		plugins: [
				new CopyWebpackPlugin([{
						...
						to: resolve(
								__dirname,
								`../${CONFIG.DIR.DIST}/${CONFIG.DIR.STYLE}`
						),
						writeToDisk: !isDev,
				}, ]),
				//写入环境变量
				new webpack.DefinePlugin({
						...
						PLUGIN_URL: "https://sw-erp.oss-accelerate.aliyuncs.com/crawl-plugin/马六甲跨境助手.crx",
						"process.env": {
				...
				.map((filepath, i) => {
						...
						const filename = ((name) =>
								`${name.split(".")[0]}.${CONFIG.EXT}`)(
								`${CONFIG.DIR.VIEW}/${tempList[tempList.length - 1]}`
						);
