...
if (isDev) {
		require("raw-loader!../views/help.ejs");
}

$(document).ready(function() {
		window.jQuery = $;
		const Pagination = require("./lib/pagination.min.js");
		...
		if (total) {
				$('.list-pagination').pagination({
						...
						callback: function(api, res) {
								...
								let searchStr = window.location.search ? window.location.search : '?';
								const href = window.location.pathname;
								if (searchStr.indexOf('&current=') > -1) {
										searchStr = searchStr.replace(/&current\=\w+/, '');
								}
								window.location = href + searchStr + '&current=' + res.pageNumber;
						}
				});
		}
})
